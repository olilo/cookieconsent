...
export default class Interface {
      ...
      if(list.length) {
        ...
        for(let item in list) {
          //listItems.push(el('li', Language.getTranslation(list[item], window.CookieConsent.config.language.current, 'name')));
            var listChildren = [];
            listChildren.push(el('h3', Language.getTranslation(list[item], window.CookieConsent.config.language.current, 'name')));

            if (typeof Language.getTranslation(list[item], window.CookieConsent.config.language.current, 'infos') !== "string") {
              var infos = Language.getTranslation(list[item], window.CookieConsent.config.language.current, 'infos');
              var rows = [];

              for (var info in infos) {
                rows.push(el('tr', el('td.' + infos[info].cname, infos[info].cname), el('td.cdesc', infos[info].cdesc), el('td.cduration', infos[info].cduration), el('td.ctype', infos[info].ctype)));
              }

                if (rows.length) {
                    listChildren.push(el('table.cookie-info', rows));
                }
            }

            listItems.push(el('li', listChildren));
        }
  ...
  addEventListeners(elements) {
    ...
    // add event listener to a DOM knot with class .ccm__cheading__open for opt out
    try {
        document.querySelector('.ccm__cheading__open').addEventListener('click', (event) => {
            event.preventDefault();
            this.elements['modal'].classList.add('ccm--visible');
            this.elements['modal'].classList.remove('ccm--hidden');
